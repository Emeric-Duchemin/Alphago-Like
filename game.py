#Mise en place du jeu pour l'entrainement MCTS
import numpy as np
import logging
import random
...
class Game:
    def __init__(self):
        _stringUnionFind = np.full((Board._BOARDSIZE**2), -1, dtype='int8')
        _stringLiberties = np.full((Board._BOARDSIZE**2), -1, dtype='int8')
        _stringSizes = np.full((Board._BOARDSIZE**2), -1, dtype='int8')
        empty = set(range(Board._BOARDSIZE **2))

        _positionHashes = np.empty((Board._BOARDSIZE**2, 2), dtype='int64')
        for x in range(Board._BOARDSIZE**2):
            for c in range(2):
                self._positionHashes[x][c] = getProperRandom()
        _currentHash = getProperRandom()
        _passHash = getProperRandom()
        ...
        for nl in [self._get_neighbors(fcoord) for fcoord in range(Board._BOARDSIZE**2)] :
             _neighborsEntries.append(len(_neighbors))
             for n in nl:
                 _neighbors.append(n)
             _neighbors.append(-1) # Sentinelle
        _neighborsEntries = np.array(self._neighborsEntries, dtype='int16')
        _neighbors = np.array(self._neighbors, dtype='int8')

        self.gameState = GameState(np.zeros(Board._BOARDSIZE**2, dtype='int8'),1,0,0,0,0,False,False,_stringUnionFind,_stringLiberties,_stringSizes,empty,_positionHashes,_currentHash,_passHash,_seenHashes,_neighbors,_neighborsEntries)


        self.actionSpace = np.zeros(Board._BOARDSIZE**2, dtype='int8')
        self.pieces = {1:'X', 0: '-', 2:'O'}
        self.grid_shape = (9,9)
        self.input_shape = (9,9,2)
        self.name = 'Go'
    ...
    def _get_neighbors(self, fcoord):
        x, y = self.unflatten(fcoord)
        neighbors = ((x+1, y), (x-1, y), (x, y+1), (x, y-1))
        return [self.flatten(c) for c in neighbors if self._isOnBoard(c[0], c[1])]
    ...
    def flatten(self,coord):
        ...
        return self._BOARDSIZE * coord[1] + coord[0]

    def unflatten(self,fcoord):
        if fcoord == -1: return (-1, -1)
        d = divmod(fcoord, self._BOARDSIZE)
        return d[1], d[0]
    ...
    def reset(self):
        ...
        self.gameState = GameState(np.zeros(Board._BOARDSIZE**2, dtype='int8'),1,0,0,0,
                                    0,False,False,_stringUnionFind,_stringLiberties,
                                    _stringSizes,empty,_positionHashes,_currentHash,_passHash,
                                    _seenHashes,_neighbors,_neighborsEntries)
...
class GameState():
    ...
    def __init__(self, board, playerTurn, nbw, nbb, cw, cb, lpass, go, suf, sl, ss,
                empt, posHash, cHash, pHash, _seenH, neigh, neighEn):
        self.board = board
        self.pieces = {1:'X', '0': '+', 2:'O'}
        ...
        self._nbWHITE = nbw #0
        self._nbBLACK = nbb #0
        self._capturedWHITE = cw #0
        self._capturedBLACK = cb #0

        self._lastPlayerHasPassed = lpass #False
        self._gameOver = go #False

        self._stringUnionFind = suf #np.full((Board._BOARDSIZE**2), -1, dtype='int8')
        self._stringLiberties = sl #np.full((Board._BOARDSIZE**2), -1, dtype='int8')
        self._stringSizes = ss #np.full((Board._BOARDSIZE**2), -1, dtype='int8')

        self._empties = empt #set(range(Board._BOARDSIZE **2))

        # Zobrist values for the hashes. I use np.int64 to be machine independant
        self._positionHashes = posHash #np.empty((Board._BOARDSIZE**2, 2), dtype='int64')
        # for x in range(Board._BOARDSIZE**2):
        #       for c in range(2):
        #           self._positionHashes[x][c] = getProperRandom()
        self._currentHash = cHash #getProperRandom()
        self._passHash = pHash #getProperRandom()

        self._seenHashes = _seenH #set()

        self._historyMoveNames = []

        #Building fast structures for accessing neighborhood
        self._neighbors = neigh #[]
        self._neighborsEntries = neighEn #[]
    ...
    def _binary(self):
        ...
        currentplayer_position = np.zeros(len(self.board), dtype='int8')
        currentplayer_position[self.board==self.playerTurn] = 1

        other_position = np.zeros(len(self.board), dtype='int8')
        other_position[self.board==3-self.playerTurn] = 1
    ...
    def _convertStateToId(self):
        player1_position = np.zeros(len(self.board), dtype='int8')
        player1_position[self.board==self.playerTurn] = 1
    ...
    def _getValue(self):
        ...
        if score[playerTurn-1] > score[2-playerTurn]:
            return (1,score[playerTurn-1],score[2-playerTurn])
        elif score[playerTurn-1] < score[2-playerTurn]:
            return (-1,score[playerTurn-1],score[2-playerTurn])
        else:
    ...
    def takeAction(self, action):
        ...
        newState = GameState(newBoard, 3-self.playerTurn,self._nbWHITE,self._nbBLACK,
                            self._capturedWHITE,self._capturedBLACK,
                            self._lastPlayerHasPassed,self._gameOver,
                            self._stringUnionFind,self._stringLiberties,self._stringSizes,
                            self._empties,self._positionHashes,self._currentHash,self._positionHashes,
                            self._seenHashes,self._neighbors,self._neighborsEntries)
        newState.play_move(action,self.playerTurn)

        value = 0
        done = 0

        if newState._gameOver:
    ...
    def render(self, logger):
    	for r in range(8):
    		logger.info([self.pieces[str(x)] for x in self.board[9*r : (9*r + 9)]])
    	logger.info('--------------')
    ...
    def play_move(self, fcoord,color):
        ...
        if fcoord != -1:  # pass otherwise
            alreadySeen, tmpHash = self._is_super_ko(fcoord, color)
            if alreadySeen:
                self._historyMoveNames.append(self.flat_to_name(fcoord))
                return False
            captured = self._put_stone(fcoord, color)
	...
	# def __init__(self, board, playerTurn,hashing,):
    #     self._currentHash = hashing
    #     self.
    ...
    # def _binary(self):
    #
    #     currentplayer_position = np.zero(Board._BOARDSIZE**2, dtype='int8')
    #     currentplayer_position[self.board==self.playerTurn] = 1
    #
    #     other_position = np.zero(Board._BOARDSIZE**2, dtype='int8')
    #     other_position[self.board==3-self.playerTurn] = 1
    #
    #     position = np.append(currentplayer_position,other_position)
    #
    ...
    #
    # def _convertStateToId(self):
    #     player1_position = np.zero(Board._BOARDSIZE**2, dtype='int8')
    #     player1_position[self.board==self.playerTurn] = 1
    #
    #     other_position = np.zero(Board._BOARDSIZE**2, dtype='int8')
    #     other_position[self.board==3-self.playerTurn] = 1
    ...
    #     if(action == -1) :
    #         newBoard = np.array(self.board)
    #         newState = GameState(newBoard, 3-self.playerTurn)
    #         if newState._lastPlayerHasPassed:
    #             newState._gameOver = True
    #         else:
    #             newState._lastPlayerHasPassed = True
    #         hash = self._passHash
    #     else :
    #         newBoard = np.array(self.board)
    # 		newState = GameState(newBoard, 3-self.playerTurn)
    #         captured = newState._put_stone(action, self._playerTurn)
    #         for fc in captured:
    #             self._capture_string(fc)
    #         newState._lastPlayerHasPassed = False
    #         if self._playerTurn == self._WHITE:
    #             newState._nbWHITE += 1
    #         else:
    #             newState._nbBLACK += 1
    #         hash = self._currentHash ^ self._getPositionHash(fcoord, color)
    #
    #
	# 	value = 0
	# 	done = 0
    #     #alreadySeen, tmpHash = self._is_super_ko(action, self._playerTurn)
    #
	# 	if newState._gameOver:
	# 		done = 1
    #
